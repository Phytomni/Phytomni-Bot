...
"""Unit tests for the brief_gene introduction LLM node.
...
section markdowns; the gene's annotation strings populate the
``brief_gene_introduction`` prompt's structured parameters on both
the gene-found and gene-not-found paths.
"""
...
async def test_introduction_node_happy_path_uses_section_context() -> None:
    ...
    # R-Intro: the Basic Genomic Information block reaches the intro LLM.
    assert "### Basic Genomic Information" in rendered_query
    assert "**Core GO Annotations**" in rendered_query


@pytest.mark.asyncio
async def test_introduction_node_gene_not_found_includes_literature() -> None:
    """gene_found=False: the intro still gets Basic Info + literature.

    Under the unified gene-not-found path the introduction summarizes
    the Basic Information block (empty annotation values) while the
    literature retrieved against the free-form query is supplied via
    the prompt's structured ``retrieve_results`` parameter.
    """
