...
"""Dual-path tests for ``KnowledgeAgent.generate_node`` chat invocation.

Pins the ``USE_CHAT_SUBGRAPH`` flag branch on the knowledge generate
node: flag-off keeps the legacy direct ``phyto_chat(...)`` call,
flag-on routes through ``_cached_chat_app().ainvoke(ChatInput)`` via
the ``knowledge_to_chat_adapters`` projection helpers. Both branches
emit the same ``main_response`` / ``final_response`` shape.
"""
