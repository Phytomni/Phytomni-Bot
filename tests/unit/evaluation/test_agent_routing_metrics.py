...
def outcome(
    ...
    *,
    ...
    if dispatchable is None:
        dispatchable = (
            predicted_agent == expected_agent and schema_valid
        )
    return RunOutcome(
        case_id=case_id,
...
def test_one_schema_valid_repeat_not_dispatchable_majority(
) -> None:
    metrics = compute_metrics(
        cases=(case("case-001", "ChatAgent"),),
        outcomes=(
...
def test_wilson_interval_handles_zero_and_full_success() -> None:
    ...
    repeated_wrong = tuple(
        outcome(
            f"case-{index:03d}", repeat, "ChatAgent", "KnowledgeAgent"
        )
        for index in range(1, 4)
