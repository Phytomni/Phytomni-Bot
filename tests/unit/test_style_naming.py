...
def test_function_docstring_waiver_is_removed_from_tests():
    ...
    violations = []
    marker = "".join(("pylint:", " disable="))
    for path in (root / "tests").rglob("*.py"):
        for line in path.read_text(encoding="utf-8").splitlines():
            if marker in line and "missing-function-docstring" in line:
...
def test_shared_checker_owns_source_suppression_inventory():
    """The shared checker, not a test-local allowlist, owns source policy."""
    root = Path(__file__).resolve().parents[2]
