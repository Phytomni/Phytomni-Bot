...
async def test_reconcile_marks_fingerprint_job_succeeded(
    ...
    async def _succeeded(t_id: str, **_: Any) -> dict[str, str]:
        assert t_id == "EI-live" or t_id == "an-fp"
    ...
    await reconcile_task("an-fp")

    job = get_latest_job(mgr_path, "b" * 64)
...
async def test_reconcile_marks_fingerprint_job_cancelled(
    ...
    register_submitted_job(
        ...
        FingerprintClaim(
            ...
            ei_task_id="EI-can",
            output_dir="/obs/out",
            claimant_task_id="an-can",
            run_id="run-can",
            user_id="alice",
        ),
    )

    async def _cancelled(t_id: str, **_: Any) -> dict[str, str]:
    ...
    await reconcile_task("an-can")

    job = get_latest_job(mgr_path, "c" * 64)
    assert job is not None
