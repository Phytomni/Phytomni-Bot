...
from typing import List, Dict, Any, Optional, Union, Literal, Tuple, TypedDict, Annotated
...
# 从 deep_genome_agents 导入所有分析函数
from .deep_genome_agents import (
    evolution_analysis,
    haplotypes_analysis,
    fst_analysis,
    enrichment_analysis,
...
class DeepGenomeAgents:
    ...
    def _build_graph(self):
        ...
        workflow.add_node("gene_annotation_node", self.run_gene_annotation_node)
        ...
        workflow.add_node("orthologs_annotation_node", self.run_orthologs_annotation_node)
        workflow.add_node("paralogs_annotation_node", self.run_paralogs_annotation_node)
        workflow.add_node("interaction_annotation_node", self.run_interaction_annotation_node)
        ...
        workflow.add_edge(START, "knowledge_node")
        ...
        workflow.add_conditional_edges(
            ...
            ["experiment_node", "introduction_node"]  # introduction_node 只在 use_analyst=False 时触发
        )
    ...
    async def arun(
        ...
        if test_mode and mock_analyst_data:
            ...
            initial_state.update({
                "analysis_tasks": mock_analyst_data.get("analysis_tasks", []),
                "analyst_summaries": mock_analyst_data.get("analyst_summaries", {}),
                "synthesize_report": mock_analyst_data.get("synthesize_report", "这是预生成的综合分析报告。"),
    ...
    async def run_analyst_node(self, state: DeepGenomeState) -> dict:
        ...
        print(f"[Analyst-{task_index}] Executing: {analysis_type} for {gene_id}")
            ...
            case 'evolution_analysis':
                try:
                    ...
                    with open(out_path / f'{gene_id}_tree.summary',
                        ...
                        summary = summary.replace('Figure 1', f'Figure {self._figure_index}')
                    ...
                    with open(out_path / f'{gene_id}_tree.legend',
                        ...
                        legend = legend.replace('Figure 1', f'Figure {self._figure_index}')
                    ...
                    if len(protein_structure_files) == 0:
                        ...
                        for structure_path in protein_structure_files:
                            ...
                            with open(out_path / f'{structure_start}.legend',
                                    'r', encoding='utf-8') as legend_file:
                                legend = legend_file.read()
                                legend = legend.replace('Table 1', f'Figure {self._figure_index}')
                                legend = legend.replace('Figure 1', f'Figure {self._figure_index}')
    ...
    async def run_knowledge_agent(self, state: DeepGenomeState):
        ...
        sorted_docs = sorted(knowledge_results,
    ...
    async def run_gene_annotation_node(self, state: DeepGenomeState):
        ...
        descruption_string = '; '.join(go['description'] for go in gene_anno.get('description', []))
        go_string = '; '.join(go['go_name'] for go in gene_anno.get('go', []))
        interpro_string = '; '.join(ip['interpro_name'] for ip in gene_anno.get('interpro', []))
        mapman_string = '; '.join(mm['mapman_description'] for mm in gene_anno.get('mapman', []))
    ...
    async def run_data_agent(self, state: DeepGenomeState):
        ...
        gene_id = state['gene_id']
        species_code = state['species_code']
        sql = ("SELECT query_gene_id, query_species, homology_gene_id, homology_species "
        ...
        sql = ("SELECT query_gene_id, query_protein, interact_gene_id, interact_protein "
        ...
        for gene_homology in gene_homology_response['data']:
            if gene_homology['homology_species'] == species_code:
                gene_paralogs_set.add((gene_homology['homology_species'], 
                                       gene_homology['homology_gene_id']))
            else:
                gene_orthologs_set.add((gene_homology['homology_species'], 
                                        gene_homology['homology_gene_id']))
        ...
        for gene_interaction in gene_interaction_response['data']:
            if gene_interaction['query_gene_id'] == gene_id:
                gene_interaction_set.add((species_code, gene_interaction['interact_gene_id']))
            elif gene_interaction['interact_gene_id'] == gene_id:
                gene_interaction_set.add((species_code, gene_interaction['query_gene_id']))
    ...
    async def _dispatch_and_wait_analysis(
        ...
        obs_output_path = output_path.split("/obs/phytomni/")[-1] if "/obs/phytomni/" in output_path else output_path
        ...
        target_file_feature = target_file_feature_map.get(analysis_type, ['.png', '.summary', '.legend'])
    ...
    async def gene_symbol(self,
        ...
        async def get_gene_symbol() -> List[str]:
            sql = ("SELECT * FROM id_table WHERE gene_id = "
            ...
            gene_symbol_list = []
            if gene_symbol_response['data'][0]['symbol'] != None:
    ...
    async def _gene_annotation(self, 
                               species_code: str, 
                               gene_id: str,
                               semaphore: Optional[asyncio.Semaphore] = None):
        async def get_gene_annotation() -> Dict:
            ...
            responses = await asyncio.gather(
                *(asyncio.to_thread(lambda sql=sql: requests.post(
                    url=self.dgc.BI_URL,
    ...
    async def run_orthologs_node(self, state: DeepGenomeState):
        ...
        species_orthologs_gene_symbol_dict = {
            species_gene: res
            for species_gene, res in zip(orthologs_species_gene_list,
                                         orthologs_gene_symbol_results)
    ...
    async def run_paralogs_node(self, state: DeepGenomeState):
        ...
        species_paralogs_gene_symbol_dict = {
            species_gene: res
            for species_gene, res in zip(paralogs_species_gene_list,
                                         paralogs_gene_symbol_results)
            if res and not isinstance(res, Exception)
        }
    ...
    async def run_interaction_node(self, state: DeepGenomeState):
        ...
        species_interaction_gene_symbol_dict = {
            species_gene: res
            for species_gene, res in zip(interaction_species_gene_list,
                                         interaction_gene_symbol_results)
            if res and not isinstance(res, Exception)
        }
    ...
    async def run_orthologs_annotation_node(self, state: DeepGenomeState):
        ...
        semaphore = asyncio.Semaphore(self.dgc.MAX_CONCURRENCY)
        orthologs_species_gene_list = state['orthologs_data']['gene_list']
        species_orthologs_gene_symbol_dict = state['orthologs_data'].get('gene_symbol', {})
        ...
        species_orthologs_gene_anno_dict = {
            species_gene: res
            for species_gene, res in zip(orthologs_species_gene_list,
                                         orthologs_gene_anno_results)
            if res and not isinstance(res, Exception)
        }

        orthologs_string = network_to_string(
            orthologs_species_gene_list, species_orthologs_gene_symbol_dict,
            species_orthologs_gene_anno_dict, 'Orthologous')
        print("Orthologous gene network summary completed <=")
        return {"orthologs_summary": orthologs_string, "part1_completed_branches": 1}
    ...
    async def run_paralogs_annotation_node(self, state: DeepGenomeState):
        ...
        semaphore = asyncio.Semaphore(self.dgc.MAX_CONCURRENCY)
        paralogs_species_gene_list = state['paralogs_data']['gene_list']
        species_paralogs_gene_symbol_dict = state['paralogs_data'].get('gene_symbol', {})
        ...
        species_paralogs_gene_anno_dict = {
            species_gene: res
            for species_gene, res in zip(paralogs_species_gene_list,
                                         paralogs_gene_anno_results)
            if res and not isinstance(res, Exception)
        }

        paralogs_string = network_to_string(
            paralogs_species_gene_list, species_paralogs_gene_symbol_dict,
            species_paralogs_gene_anno_dict, 'Paralogous')
        print("Paralogous gene network summary completed <=")
        return {"paralogs_summary": paralogs_string, "part1_completed_branches": 1}
    ...
    async def run_interaction_annotation_node(self, state: DeepGenomeState):
        ...
        semaphore = asyncio.Semaphore(self.dgc.MAX_CONCURRENCY)
        interaction_species_gene_list = state['interaction_data']['gene_list']
        species_interaction_gene_symbol_dict = state['interaction_data'].get('gene_symbol', {})
        ...
        species_interaction_gene_anno_dict = {
            species_gene: res
            for species_gene, res in zip(interaction_species_gene_list,
                                         interaction_gene_anno_results)
            if res and not isinstance(res, Exception)
        }

        interaction_string = network_to_string(
            interaction_species_gene_list, species_interaction_gene_symbol_dict,
            species_interaction_gene_anno_dict, 'Potential interacting')
        print("Interacting gene network summary completed <=")
        return {"interaction_summary": interaction_string, "part1_completed_branches": 1}
    ...
    async def run_part1_node(self, state: DeepGenomeState):
        ...
        print('part1_completed_branches: ', state.get("part1_completed_branches", 0))
        ...
        user_query = get_prompt(
            self.dgc.PROMPT_FILE, 'user/gene_function_network_anno', prompt_vars)

        phyto_response = await phyto_chat(
            user_query=user_query,
            prompt_file=self.dgc.PROMPT_FILE,
            prompt_path=self.dgc.PROMPT_PATH,
            api_key=sc.API_KEY.get_secret_value(),
        ...
        if (phyto_response and 'choices' in phyto_response and
            ...
            part1_report = phyto_response['choices'][0]['message'].get('content', '')
        else:
            part1_report = f"Gene {gene_id} basic profile generation failed"

        return {"part1_report": part1_report, "experiment_completed_branches": 1}
    ...
    async def run_report_synthesizer(self, state: DeepGenomeState):
        ...
        if state.get("skip_synthesize", False):
            print("  [Test Mode] Skipping synthesize_node, using pre-prepared mock data")
            return {"experiment_completed_branches": 1}

        if total_expected > 0 and completed < total_expected:
            print(f"  [Barrier] Waiting for analysis completion: {completed}/{total_expected}")
            return {}

        print(f"  [Barrier] All analysis completed ({completed}/{total_expected}), starting synthesis...")

        gene_results_data = state.get('analyst_summaries', {})
        gene_results = get_prompt(self.dgc.PROMPT_FILE,
        ...
        for obj_key, replace_content in obj_replace_dict.items():
            try:
                ...
                continue
        with open(f'{self.dgc.DEEPGENOME_OUT}/{state['gene_id']}_results.md',
              'w', encoding='utf-8') as fo:
            fo.write(gene_results)
        # print(gene_results)
        return {"synthesize_report": gene_results, "experiment_completed_branches": 1}
    ...
    async def run_report_experiment(self, state: DeepGenomeState):
        ...
        if gene_results_data:
            part2_str = get_prompt(self.dgc.PROMPT_FILE,
                                   'template/gene_function_result',
                                   gene_results_data)
        else:
            part2_str = ''
        ...
        function_experiment = ''
        if (experiment_response and 'choices' in experiment_response and
                len(experiment_response['choices']) > 0 and
                'message' in experiment_response['choices'][0]):
            function_experiment = experiment_response['choices'][0]['message'].get('content', '')
        ...
        for ei, experiment in enumerate(experiment_list):
            ...
            protocol_content = ''
            if (protocol_response and 'choices' in protocol_response and
                    len(protocol_response['choices']) > 0 and
                    'message' in protocol_response['choices'][0]):
                protocol_content = protocol_response['choices'][0]['message'].get('content', '')
    ...
    async def run_report_introduction(self, state: DeepGenomeState):
        ...
        if (introduction_response and 'choices' in introduction_response and
                len(introduction_response['choices']) > 0 and
                'message' in introduction_response['choices'][0]):
            introduction_content = introduction_response['choices'][0]['message'].get('content', '')
    ...
    async def run_report_discussion(self, state: DeepGenomeState):
        ...
        discussion_content = ''
        if (discussion_response and 'choices' in discussion_response and
                len(discussion_response['choices']) > 0 and
                'message' in discussion_response['choices'][0]):
            discussion_content = discussion_response['choices'][0]['message'].get('content', '')
    ...
    async def run_report_summary(self, state: DeepGenomeState):
        ...
        summary_content = ''
        if (summary_response and 'choices' in summary_response and
                len(summary_response['choices']) > 0 and
                'message' in summary_response['choices'][0]):
            summary_content = summary_response['choices'][0]['message'].get('content', '')
    ...
    async def run_follow_up_node(self, state: DeepGenomeState):
        ...
        follow_up_response = await phyto_chat(
            user_query=get_prompt(
                self.dgc.PROMPT_FILE, 'system/follow_up_questions',
                {
            ...
            if species_gene in species_gene_anno_dict:
                ...
                if 'go' in gene_anno:
                    for go_list in gene_anno['go']:
                        go_id_dict.update({go_list['go_id']: go_list['go_name']})
                ...
                if 'interpro' in gene_anno:
                    for ip_list in gene_anno['interpro']:
                        ip_id_dict.update({ip_list['interpro_id']: ip_list['interpro_name']})
                ...
                if 'mapman' in gene_anno:
                    for mm_list in gene_anno['mapman']:
                        mm_id_dict.update({mm_list['mapman']: mm_list['mapman_description']})
