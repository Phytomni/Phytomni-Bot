...
async def gene_network(
    ...
    """
    message_content = (
        "What is the query_gene_id_10, query_species_10, "
    ...
    message_content = (
        "Give the query_gene_id_11, query_protein_11, "
    ...
    gene_homology_response, gene_interaction_response = await asyncio.gather(
        homology_task, interaction_task
    )
...
async def gene_symbol(
    ...
    async def get_gene_symbol() -> Dict:
        message_content = (
            f"List all the columns whose gene_id_1 is {gene_id}"
...
async def gene_function(
    ...
    async def _execute_pipeline(server_task_id: str) -> Dict[str, Any]:
        ...
        if use_analyst_agent:
            ...
            species_str = (
                SPECIES_CODE_MAP[species_code].split("(")[1].strip(")").lower()
            )
            ...
            manager.update_task(
                server_task_id, "running", gene_task_str, output_dir
            )
        ...
        results = await asyncio.gather(
            *(symbol_tasks + anno_tasks), return_exceptions=True
        )
        ...
        gene_string = "|".join(
            species_gene_symbol_dict.get((species_code, gene_id), [])
        )
        if use_data_agent:
            ...
            prompt_vars = {
                ...
                "interpro_string": "; ".join(
                    ip[1] for ip in gene_anno.get("interpro", [])
                ),
                "mapman_string": "; ".join(
                    mm[1] for mm in gene_anno.get("mapman", [])
                ),
        ...
        if use_analyst_agent:
            ...
            function_experiment = experiment_response["choices"][0]["message"][
                "content"
            ]
            ...
            for ei, experiment in enumerate(experiment_list):
                ...
                protocol_content = protocol_response["choices"][0]["message"][
                    "content"
                ]
            ...
            manager.update_task(
                server_task_id, "finished", gene_task_str, output_dir
            )
        else:
...
async def evolution_analysis(
    ...
    goal_description = get_prompt(
        prompt_file, "user/evolution_analysis", {"gene_id": gene_id}
    )
...
async def protein_structure_analysis(
    ...
    """
    goal_description = get_prompt(
        prompt_file, "user/structure_analysis", {"gene_id": gene_id}
    )
    data_list = get_data_list(deepgenome_data, "structure_analysis", species)
    if not batch:
        if not user_id:
            user_id = str(uuid1())
        output_dir = create_output_dir(
...
async def promoter_analysis(
    ...
    """
    goal_description = get_prompt(
        prompt_file, "user/promoter_analysis", {"gene_id": gene_id}
    )
...
async def gene_expression_tissues(
    ...
    data_list = get_data_list(
        deepgenome_data, "gene_expression_analysis", species
    )["tissues"]
    if not batch:
        if not user_id:
            user_id = str(uuid1())
...
async def gene_expression_cultivars(
    ...
    goal_description = get_prompt(
        prompt_file,
        "user/gene_expression_analysis/cultivar",
        {"gene_id": gene_id},
    )
    data_list = get_data_list(
        deepgenome_data, "gene_expression_analysis", species
...
async def gene_expression_genotypes(
    ...
    goal_description = get_prompt(
        prompt_file,
        "user/gene_expression_analysis/genotype",
        {"gene_id": gene_id},
    )
    data_list = get_data_list(
        deepgenome_data, "gene_expression_analysis", species
...
async def gene_expression_treatments(
    ...
    goal_description = get_prompt(
        prompt_file,
        "user/gene_expression_analysis/treatment",
        {"gene_id": gene_id},
    )
    data_list = get_data_list(
        deepgenome_data, "gene_expression_analysis", species
...
async def single_cell_analysis(
    ...
    """
    goal_description = get_prompt(
        prompt_file, "user/single_cell_analysis", {"gene_id": gene_id}
    )
...
async def smoc_analysis(
    ...
    """
    goal_description = get_prompt(
        prompt_file, "user/smoc_analysis", {"gene_id": gene_id}
    )
    data_list = get_data_list(deepgenome_data, "promoter_analysis", species)
    if not batch:
        if not user_id:
...
async def haplotypes_analysis(
    ...
    """
    goal_description = get_prompt(
        prompt_file, "user/haplotypes_analysis", {"gene_id": gene_id}
    )
...
async def fst_analysis(
    ...
    msu_id = next(
        item["cell_value"]
        for item in response["query_data"][1]
...
async def summarize_gene_analysis(
    ...
    target_map = {
        ...
        "protein_structure_task": ["sample_0.cif", ".summary", ".legend"],
        "promoter_task": ["motif_all_logo.png", ".summary", ".legend"],
        "fst_task": [".png"],
    ...
    try:
        ...
        with open(
            out_path / f"{gene_id}_tree.summary", "r", encoding="utf-8"
        ) as summary_file:
            summary = summary_file.read()
            summary = summary.replace("Figure 1", f"Figure {figure_index}")
            gene_results_data["tree_summary"] = summary
        with open(
            out_path / f"{gene_id}_tree.legend", "r", encoding="utf-8"
        ) as legend_file:
    ...
    try:
        with open(
            out_path / f"{gene_id}_domain.md", "r", encoding="utf-8"
        ) as domain_f:
            domain = domain_f.read()
            gene_results_data["domain_table"] = domain
        with open(
            out_path / f"{gene_id}_domain.summary", "r", encoding="utf-8"
        ) as summary_file:
            summary = summary_file.read()
            gene_results_data["domain_summary"] = summary
        with open(
            out_path / f"{gene_id}_domain.legend", "r", encoding="utf-8"
        ) as summary_file:
            summary = summary_file.read()
    ...
    try:
        ...
        with open(
            out_path / f"{gene_id}_single_cell.summary", "r", encoding="utf-8"
        ) as summary_file:
            summary = summary_file.read()
            gene_results_data["single_cell_summary"] = summary
        with open(
            out_path / f"{gene_id}_single_cell.legend", "r", encoding="utf-8"
        ) as legend_file:
            legend = legend_file.read()
            legend = legend.replace("Figure 1", f"Figure {figure_index}")
    ...
    try:
        ...
        with open(
            out_path / f"{gene_id}_haplotype.summary", "r", encoding="utf-8"
        ) as summary_file:
            summary = summary_file.read()
            gene_results_data["haplotype_summary"] = summary
        with open(
            out_path / f"{gene_id}_haplotype.legend", "r", encoding="utf-8"
        ) as legend_file:
            legend = legend_file.read()
            legend = legend.replace("Figure 1", f"Figure {figure_index}")
    ...
    try:
        ...
        with open(
            out_path / f"{gene_id}_motif.summary", "r", encoding="utf-8"
        ) as summary_file:
            summary = summary_file.read()
            summary = summary.replace("Figure 1", f"Figure {figure_index}")
            gene_results_data["motif_summary"] = summary
        with open(
            out_path / f"{gene_id}_motif.legend", "r", encoding="utf-8"
        ) as legend_file:
            legend = legend_file.read()
            legend = legend.replace("Figure 1", f"Figure {figure_index}")
    ...
    if (
        gene_results_data["smep_summary"] == ""
        and gene_results_data["smoc_summary"] == ""
    ):
            ...
            for structure_path in protein_structure_files:
                ...
                with open(
                    ...
                    legend = legend.replace(
                        "Table 1", f"Figure {figure_index}"
                    )
                    legend = legend.replace(
                        "Figure 1", f"Figure {figure_index}"
                    )
    ...
    gene_results = get_prompt(
        prompt_file, "template/gene_function_result", gene_results_data
    )
    obj_replace_dict = {
    ...
    for obj_key, replace_content in obj_replace_dict.items():
        try:
            ...
            continue
    with open(
        f"{deepgenome_out}/{gene_id}_results.md", "w", encoding="utf-8"
    ) as fo:
