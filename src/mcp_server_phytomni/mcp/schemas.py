...
class PhytomniAgents(StrEnum):
    ...
    KNOWLEDGE_AGENT_DESCRIPTION = (
        "Use for a focused plant-science question whose answer must be "
        "grounded in retrieved literature, patents, books, or supplied "
        "documents. Return a sourced answer to the specific question; do "
        "not choose this for broad multi-section reviews, structured "
        "database results, paper-analysis reproduction, or computational "
        "task submission."
    )
    ...
    BRIEF_GENE_AGENT_DESCRIPTION = (
        "Use when the user asks what one identified plant gene or "
        "transcript does and wants a concise function report combining "
        "available annotations with supporting literature. The request "
        "should contain an accepted database gene ID; do not choose this "
        "for database table queries, comprehensive computational gene "
        "analysis, sequence design, or multi-gene review."
    )
    DEEP_GENOME_AGENT = "DeepGenomeAgent"
    DEEP_GENOME_AGENT_DESCRIPTION = (
        "Use when the user requests a comprehensive computational analysis "
        "of one plant gene using species_code and gene_id, including "
        "functional context, homologs, interactions, experiments, "
        "protocols, and a detailed report. Submit the deep gene-analysis "
        "pipeline; do not choose this for a concise function summary, "
        "promoter or protein design, or general literature Q&A."
    )
    ...
    DIGITAL_DESIGN_AGENT_DESCRIPTION = (
        "Use when the user requests computational optimization or design "
        "of both promoter and protein sequences for one gene, identified "
        "by species_code and gene_id. Submit the dedicated design analyses; "
        "do not choose this for explaining gene function, comprehensive "
        "gene characterization, ordinary sequence analysis, or literature "
        "retrieval."
    )
    GENE_NETWORK_AGENT = "GeneNetworkAgent"
    GENE_NETWORK_AGENT_DESCRIPTION = (
        "Use when the user wants to engineer a concrete plant trait by "
        "identifying or analyzing trait-associated gene networks for a "
        "species, with species_code and an active Trait Ontology to_id. "
        "Submit the trait-network analysis; do not choose this for general "
        "hormone explanations, free-form interaction questions, literature "
        "reviews, or single-gene function reports."
    )
