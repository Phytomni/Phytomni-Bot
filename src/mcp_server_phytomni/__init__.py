...
try:
    ...
    # No on-disk config source (no plaintext .env, no encrypted
    # envelope). Either the environment was supplied directly (e.g.
    # docker -e) or an auxiliary entry point needs no secrets (e.g. the
    # phytomni-cache CLI over local SQLite). The authoritative
    # "refuse to boot with empty secrets" guard stays at the
    # secret-loading seam (get_sensitive_config), which re-runs this
    # same check when an agent actually needs credentials.
    pass
