...
def resolve_scratch_dir(
    ...
    """
    if obsfs_bucket_available(bucket_name, obsfs_mount_root):
        try:
            return _resolve_obsfs_scratch_dir(
                kind,
                run_identity,
                task,
                bucket_name,
            ...
            pass
    return _resolve_local_scratch_dir(run_identity, task, local_fallback)
...
def _resolve_obsfs_scratch_dir(
    ...
    obsfs_path_for(key, bucket_name, obsfs_mount_root).mkdir(
        parents=True, exist_ok=True
    )
    return obs_path_from_key(bucket_name, key)
