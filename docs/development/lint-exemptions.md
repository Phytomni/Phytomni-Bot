...
Counted instead by the L2 ratchet. The current baseline is 36; the
37th fake pushes the count to 37, the ratchet fails, refactor or
explicit baseline bump is forced.

**A 159th R0801 duplicate-code violation appears.** Default pylint
would emit a warning but the gate uses similar-lines tolerance.
Counted instead by the L2 ratchet. The current baseline is 158; the
159th duplicate fails.
...
### Test fakes and Protocol stubs — too-few-public-methods (36 occurrences)

**Rule(s)**: R0903 too-few-public-methods (1/2). 36 occurrences (the
...
- `src/mcp_server_phytomni/agents/deep_genome/brief_gene_mount.py:160`
  (`DeepGenomeBriefGeneMountMixin`)
- `src/mcp_server_phytomni/graphs/loader.py:34` (`GraphLoader`)
- `tests/agents/test_deep_genome_brief_gene_mount.py:260 + 283`
  (`_BrokenApp` mount fakes)
- `tests/agents/test_deep_genome_lifecycle.py:176 + 443`
  (required-mount and design-failure fakes)
- `tests/server/test_a2ui_review_http.py` (checkpointer and review-app fakes)
- `tests/server/test_resume_mcp.py` (`_FakeInterrupt`, `_FakeSession`, and
  elicitation result)
...
`scripts/check_pylint_baseline.py` (`RULE_BASELINES["R0903"] = 36`).
...
### R0801 duplicate-code (legacy catalog; 158 current occurrences)
...
count is 158 and is enforced by `check_pylint_baseline.py`.
...
`RULE_BASELINES["R0801"] = 158`). The catalog header count above
...
and 124 (A2A/interop/memory rollout). The latest +34 windows were
measured after the DeepGenome lifecycle/report, streaming/A2UI, Gauss
safety, and external-handoff contracts landed. They are deliberate
parallels across request, state, and response surfaces; extracting them
into a shared test or domain abstraction would hide the contract each
surface is meant to exercise. The current count is 158 and is the
authoritative value enforced by the ratchet.
