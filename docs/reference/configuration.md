...
| `MEMORY_ENABLED`             | `false`                           | no         | Opt-in user-scoped memory CRUD routes; accepts `PHYTOMNI_MEMORY_ENABLED`. Disabled deployments do not mount the routes or open the memory database.                                                                                                      |
| `MEMORY_DB_PATH`             | `.cache/phytomni/memory.sqlite`   | no         | Local SQLite path for memory records; accepts `PHYTOMNI_MEMORY_DB_PATH` and should point to a persistent local filesystem when enabled.                                                                                                                  |
