...
The complete public GET and POST contract is the
[HTTP API Endpoints](../reference/http-api.md#endpoints)
inventory. This runbook does not duplicate that list.

Operator probes used in the deployment checklist:
...
- **Method:** `GET`
  ...
  **Auth:** yes
  **Operational use:** Bounded V1-compatible event history.

- **Method:** `GET`
  **Path:** `/v1/runs/{run_id}/events/{event_id}`
  **Auth:** yes
  **Operational use:** One owner-authorized compatibility event.

- **Method:** `GET`
  **Path:** `/v1/runs/{run_id}/events/stream`
  **Auth:** yes
  **Operational use:** Resumable compatibility event stream.

- **Method:** `GET`
  **Path:** `/v1/runs/{run_id}/event-projection`
  **Auth:** yes
  **Operational use:** Current bounded compatibility projection.

- **Method:** `GET`
  **Path:** `/v1/executions/{execution_id}/events`
  **Auth:** yes
  **Operational use:** Bounded canonical V2 event history.

- **Method:** `GET`
  **Path:** `/v1/executions/{execution_id}/events/{event_id}`
  **Auth:** yes
  **Operational use:** One owner-authorized V2 event.

- **Method:** `GET`
  **Path:** `/v1/executions/{execution_id}/events/stream`
  **Auth:** yes
  **Operational use:** Resumable V2 event/content stream with heartbeats.

- **Method:** `GET`
  **Path:** `/v1/executions/{execution_id}/event-projection`
  **Auth:** yes
  **Operational use:** Current deterministic V2 execution projection.
...
`Expert routing provider completed` (`outcome`, `path`, `duration_ms`);
they do not change the public codes. Synchronous selections
