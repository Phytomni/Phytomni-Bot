...
The isolated-worktree follow-up after `e6bb25d` passed `102` focused
context-history tests. It proves Bot-owned conversation context now retains a
bounded chronological native-turn stream, derives the legacy split arrays
backward-compatibly, preserves unmatched degraded user turns and repeated text,
and excludes only the rebuilt trailing current-user slot by position before
dispatch. This is Bot-local evidence only: public API/MCP schemas, agent
selection behavior, and external Web/Go acceptance remain unchanged.
...
| Section 12 context-history follow-up | Spec 12; Expert Task 6              | `e6bb25d` + follow-up commit               | local       | context-history packet     | 102 focused passed         | chronological native turns | Expert plan     | Needs Verification | external acceptance absent   | keep route dark             |
