...
| `PHYTOMNI_USE_KNOWLEDGE_SUBGRAPH` / `USE_KNOWLEDGE_SUBGRAPH` | `false` | Per-deployment opt-in for consumer agents that invoke knowledge retrieval. When `true`, every consumer (analyst / data / review) routes its `retrieve_node` through `KnowledgeAgent`'s compiled subgraph entry point via `adapter_node` instead of the legacy inline `multi_retrieve` / `retrieve` helpers. Review additionally requires `USE_CHAT_SUBGRAPH=true` and fans out per research dimension through a Send-dispatch worker. Default `false` preserves current behavior. |
| `PHYTOMNI_USE_EVOLUTION_SUBGRAPH` / `USE_EVOLUTION_SUBGRAPH` | `false` | Per-deployment opt-in for the deep_genome dispatch's `evolution_analysis` branch. When `true`, deep_genome routes the branch through the module-level `evolution_analysis_for_gene` producer wrapper (which itself dispatches through `submit_analyst_via_subgraph`) instead of building the analyst request inline. Lives on `DeepGenomeConfig` because no sibling agent dispatches `evolution_analysis`. Default `false` preserves current behavior.                            |
| `PHYTOMNI_USE_DESIGN_SUBGRAPH` / `USE_DESIGN_SUBGRAPH`       | `false` | Per-deployment opt-in for the deep_genome dispatch's `protein_structure_analysis` and `promoter_analysis` branches. When `true`, deep_genome routes both branches through the module-level `protein_structure_for_gene` / `promoter_design_for_gene` producer wrappers instead of building the analyst request inline. Lives on `DeepGenomeConfig` for the same single-consumer reason as the evolution flag. Default `false` preserves current behavior.                         |
