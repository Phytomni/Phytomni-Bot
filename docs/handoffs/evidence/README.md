...
1. A local plan checkbox, mock response, or screenshot without a request ID is
   not evidence.
2. A `Closed` state requires a reviewed artifact and a link from the tracked
   disposition matrix; the template intentionally does not use `Closed`.
3. A failed check gets a new record and keeps the previous record immutable.
4. Any credential-shaped value, customer data, or private endpoint rejects the
   record until it is redacted.
5. Missing owner output remains `External Pending`.
