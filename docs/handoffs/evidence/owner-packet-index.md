...
1. Replace synthetic values with sanitized observations and retain the exact
   commit/environment labels.
1. Attach the record to the owner handoff, without editing the matrix status.
1. The Bot maintainer checks the record shape, redaction, and acceptance rule.
1. Only the maintainer links reviewed evidence and changes the matrix state.
